"""Run a frozen 16-question D/E/F answer ablation with resumable checkpoints."""
...
def main():
    ...
    if not dataset_path.exists():
        ...
        ids = {"public-011", "public-040", "public-043", "public-058", "public-064", "public-065",
               "public-074", "public-075", "public-077", "public-079", "public-080", "public-082",
               "public-083", "public-084", "public-088", "public-099"}
        source["questions"] = [q for q in source["questions"] if q["id"] in ids]
        if len(source["questions"]) != 16:
            raise ValueError("Selected questions are missing")
        write_json(dataset_path, source)
    ...
    if manifest_path.exists():
        ...
        manifest = {
            ...
            "interpretation": "预先固定16题：2文本、4表格、8图表、2无答案；偏重视觉及已知困难题的诊断消融，非独立泛化测试。评分另存，原始评分保持空值。",
            "runs": {},
        }
        write_json(manifest_path, manifest)
    for label, split, groups, generate in [
        ("visual-ablation", "test", list("DEF"), True),
    ]:
    ...
    print("ABLATION COMPLETE; separate answer review is pending.", flush=True)
